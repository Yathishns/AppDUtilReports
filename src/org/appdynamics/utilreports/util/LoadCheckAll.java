 ...
 */
public class LoadCheckAll implements Runnable{
    private static Logger logger=Logger.getLogger(LoadCheckAll.class.getName());
    private String name,appId, header,sheetName;
    ...
    public LoadCheckAll(String name, RESTAccess access){this.name=name;this.access=access;}
    public LoadCheckAll(String name, RESTAccess access,ArrayList<Check> checks){this.name=name;this.access=access;this.checks=checks;}
    ...
    public void sortChecks(){
        Collections.sort(checks,new Comparator<Check>(){
            @Override
            public int compare (final Check chk1, final Check chk2){
